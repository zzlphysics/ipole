...
def run_c(a, thetacam, Ne_unit, Te_unit, disk_h, MBH, pow_nth, pow_T, keplerian_factor, infall_factor, fluid_dirction, rotcam, xoff, yoff, outfile):
    cmd = [
...
def run_c_unpol(a, thetacam, Ne_unit, Te_unit, disk_h, MBH, pow_nth, pow_T, keplerian_factor, infall_factor, fluid_dirction, rotcam, xoff, yoff, outfile):
    cmd = [
        './ipole', '-unpol', '-quench', '-par', 'm87.par',
        '--a={}'.format(a),
        '--thetacam={}'.format(thetacam),
        '--Ne_unit={}'.format(Ne_unit),
        '--Te_unit={}'.format(Te_unit),
...
if __name__ == '__main__':
    ...
    Te_unit_range = (1e12, 1e13)
    disk_h_range = (0.8, 0.95)
    MBH_range = (1e9, 5e9)
    ...
    for _ in range(run_count):  # 生成数据集
        ...
        rotcam = 0 #random.uniform(*rotcam_range)
        ...
        nll = lambda args:run_c_unpol(a,thetacam,args,Te_unit,disk_h,MBH,pow_nth,pow_T,keplerian_factor,
                                      infall_factor,fluid_dirction,rotcam,xoff,yoff,outfile)-0.5
            
        try:
            soln = root_scalar(nll, bracket=[1e2, 1e9], method='brentq', xtol = 5e-2,rtol = 2e-2, maxiter=30)
        ...
        parsed_output = run_c(a,thetacam,soln.root,Te_unit,disk_h,MBH,pow_nth,pow_T,keplerian_factor,infall_factor,fluid_dirction,rotcam,xoff,yoff,outfile)
        sys.stdout.flush()
