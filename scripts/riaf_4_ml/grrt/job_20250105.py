...
def run_c(a, thetacam, Ne_unit, Te_unit, disk_h, MBH, pow_nth, pow_T, keplerian_factor, infall_factor, fluid_dirction, rotcam, xoff, yoff, pol_resulation, outfile):
    cmd = [
        './ipole', '-par', 'sgra.par',
        '--a={}'.format(a),
        ...
        '--xoff={}'.format(xoff*pol_resulation),
        '--yoff={}'.format(yoff*pol_resulation),
        '--outfile={}'.format(h5file_dir + "/" + outfile),
        '--nx={}'.format(pol_resulation),
        '--ny={}'.format(pol_resulation)
    ]
...
def run_c_unpol(a, thetacam, Ne_unit, Te_unit, disk_h, MBH, pow_nth, pow_T, keplerian_factor, infall_factor, fluid_dirction, rotcam, xoff, yoff, unpol_resulation, outfile):
    cmd = [
        './ipole', '-unpol', '-quench', '-par', 'sgra.par',
        '--a={}'.format(a),
        '--thetacam={}'.format(thetacam),
        '--Ne_unit={}'.format(Ne_unit),
        '--Te_unit={}'.format(Te_unit),
        ...
        '--fluid_dirction={}'.format(fluid_dirction),
        '--rotcam={}'.format(rotcam),
        '--xoff={}'.format(xoff*unpol_resulation),
        '--yoff={}'.format(yoff*unpol_resulation),
        '--nx={}'.format(unpol_resulation),
        '--ny={}'.format(unpol_resulation)
    ]
...
# 添加命令行参数
parser.add_argument('--run_count', type=int, help='The number of runs')
...
if __name__ == '__main__':
    ...
    unpol_resulation = 100
    pol_resulation = 256

    # 定义参数范围
    a_range = (-0.999, 0.999)  # 范围 (min, max)
    thetacam_range = (1,179)
    # Ne_unit_range = (1e4, 1e8)
    Te_unit_range = (1e10, 1e12)
    disk_h_range = (0.1, 0.8)
    MBH_range = (3.8e6, 4.2e6)
    # pow_nth_range = (-1.4, -0.8)
    # pow_T_range = (-1.14, -0.54)
    ...
    df = pd.DataFrame(columns=['filename', 'a', 'thetacam', 'Ne_unit', 'Te_unit', 'disk_h', 'MBH', 'pow_nth', 
                            'pow_T', 'keplerian_factor', 'fluid_dirction', 'rotcam', 'xoff', 'yoff', 
    ...
    # 运行ipole并保存参数到CSV
    for _ in range(run_count):  # 生成数据集
        ...
        pow_nth = -1.1 #random.uniform(*pow_nth_range)
        pow_T = -0.84 #random.uniform(*pow_T_range)
        keplerian_factor = random.uniform(*keplerian_factor_range)
        infall_factor = 1.0 - keplerian_factor
        fluid_dirction = 1.0 #random.choice(fluid_dirction_range)
        rotcam = random.uniform(*rotcam_range)
        xoff = random.uniform(*xoff_range)
        yoff = random.uniform(*yoff_range)
        ...
        print('rotcam:', rotcam)
        print('xoff:', xoff*pol_resulation)
        print('yoff:', yoff*pol_resulation)
        print('outfile:', outfile)

        nll = lambda args:run_c_unpol(a,thetacam,args,Te_unit,disk_h,MBH,pow_nth,pow_T,keplerian_factor,
                                      infall_factor,fluid_dirction,rotcam,xoff,yoff,unpol_resulation,outfile)-2.4
            
        try:
            soln = root_scalar(nll, bracket=[1e3, 1e8], method='brentq', xtol = 5e-2,rtol = 2e-2, maxiter=30)
        ...
        parsed_output = run_c(a,thetacam,soln.root,Te_unit,disk_h,MBH,pow_nth,pow_T,keplerian_factor,infall_factor,fluid_dirction,rotcam,xoff,yoff,pol_resulation,outfile)
        sys.stdout.flush()
        ...
        new_data = {
            ...
            'rotcam': rotcam,
            'xoff': xoff*pol_resulation,
            'yoff': yoff*pol_resulation,
            **parsed_output
        }
