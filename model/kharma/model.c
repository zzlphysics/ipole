...
double get_dump_t(char *fnam, int dumpidx)
{
  ...
  } else if (dumpfile_format == FORMAT_KHARMA) {
    hdf5_read_single_val(&t, "Info/Time", H5T_IEEE_F64LE);
  }
  hdf5_close();
...
void get_dumpfile_type(char *fnam, int dumpidx)
{
  ...
  if (!hamr_attr_exists) {
    if (hdf5_exists("Input")) {
      dumpfile_format = FORMAT_KHARMA;
      fprintf(stderr, "kharma!\n");
    } else if (!hdf5_exists("header/version")) {
...
void init_iharm_grid(char *fnam, int dumpidx)
{
  ...
  if ( strncmp(metric_name, "MKS", 19) == 0) {
    metric = METRIC_MKS;
    cstopx[2] = 1.0;
    // cstopx设置的是在native坐标情况下的坐标范围，这里就是mks情况下的
  ...
  // 置电子温度模型
  // 我们可以在这里覆盖电子模型。如果覆盖，则打印结果。ELECTRONS应该只在需要使用
  // 额外变量（而不是仅仅UU和RHO）来thetae时才不为零。
  // we can override which electron model to use here. print results if we're
  // overriding anything. ELECTRONS should only be nonzero if we need to make
  // use of extra variables (instead of just UU and RHO) for thetae
  if (!USE_FIXED_TPTE && !USE_MIXED_TPTE) {
  ...
  // 通过这一点，我们确定Thetae_unit是我们想要的，所以我们可以设置
  // by this point, we're sure that Thetae_unit is what we want so we can set
  // Te_unit which is what ultimately get written to the dump files
  Te_unit = Thetae_unit;
...
// 添加辅助函数来定位section的范围
static char* find_section_start(const char *par_string, const char *section) {
  char search_str[256];
  snprintf(search_str, sizeof(search_str), "<%s>", section);
  return strstr(par_string, search_str);
}

static const char* find_section_end(const char *section_start) {
  if (section_start == NULL) return NULL;
  const char *end = strstr(section_start + 1, "<");
  return end ? end : section_start + strlen(section_start);
}

// 在section范围内查找参数值
int get_param_int(const char *par_string, const char *section, const char *param_name, int default_value) {
  char *start = find_section_start(par_string, section);
  if (start == NULL) {
    fprintf(stderr, "Section %s not found, using default value %d for %s\n", 
            section, default_value, param_name);
    return default_value;
  }
  
  const char *end = find_section_end(start);
  
  char search_str[256];
  snprintf(search_str, sizeof(search_str), "\n%s", param_name);
  char *pos = strstr(start, search_str);
  
  if (pos == NULL || pos >= end) {
    fprintf(stderr, "Parameter %s not found in section %s, using default value %d\n", 
            param_name, section, default_value);
    return default_value;
  }
...
double get_param_double(const char *par_string, const char *section, const char *param_name, double default_value) {
  char *start = find_section_start(par_string, section);
  if (start == NULL) {
    fprintf(stderr, "Section %s not found, using default value %g for %s\n", 
            section, default_value, param_name);
    return default_value;
  }
  ...
  if (pos == NULL || pos >= end) {
    fprintf(stderr, "Parameter %s not found in section %s, using default value %g\n", 
            param_name, section, default_value);
    return default_value;
  }
...
// char* get_param_string(const char *par_string, const char *section, const char *param_name, const char *default_value, char *result, size_t result_size) {
    
//     char *start = find_section_start(par_string, section);
//     if (start == NULL) {
//       fprintf(stderr, "Section %s not found, using default value %s for %s\n", 
//               section, default_value, param_name);
//         strncpy(result, default_value, sizeof(result)-1);
//         return result;
//     }
...
//     char *line = start - 1;
//     char *pos = NULL;
//     while (line < end && (line = strchr(line, '\n')) != NULL) {
//         line++; // 跳过换行符
//         // 跳过行首空格
...
void get_param_string(const char *par_string, const char *section, const char *param_name, const char *default_value, char *result, size_t result_size) {
    char *start = find_section_start(par_string, section);
    if (start == NULL) {
        fprintf(stderr, "Section %s not found, using default value %s for %s\n",
                section, default_value, param_name);
    ...
    // Modify search to allow for spaces before the parameter name
    char *line = start - 1;
    char *pos = NULL;
    while (line < end && (line = strchr(line, '\n')) != NULL) {
    ...
    // Skip spaces after parameter name
    while (*pos == ' ' || *pos == '\t') pos++;
...
void init_kharma_grid(char *fnam, int dumpidx)
{
  ...
  // 修改读取Input参数的方式
  char par_string[32768];  // 增大到64KB
  
  // 1. 首先检查Input group是否存在
  if (!hdf5_exists("/Input")) {
    fprintf(stderr, "Error: /Input group not found\n");
    exit(-1);
  }

  // 2. 设置当前目录到Input group 
  hdf5_set_directory("/Input");

  // 3. 检查File属性是否存在 - 使用空字符串作为相对路径
  if (!hdf5_attr_exists("", "File")) {
    fprintf(stderr, "Error: File attribute not found in Input group\n");
    exit(-1);
  }
  fprintf(stderr, "File attribute found in Input group\n");

  // 4. 读取File属性 - 使用空字符串作为相对路径
  if (hdf5_read_attr_str(par_string, "File", "", 32768) < 0) {
    fprintf(stderr, "Error reading File attribute from Input group\n");
    exit(-1);
  }

  // 打印读取的内容用于调试
  fprintf(stderr, "Successfully read Input/File attribute\n");
  // fprintf(stderr, "par_string content: %.100s...\n", par_string);

  // 读取坐标系统信息
  char base[256];
  char transform[256];
  get_param_string(par_string, "coordinates", "base", "spherical_ks", base, sizeof(base));
  get_param_string(par_string, "coordinates", "transform", "mks", transform, sizeof(transform));

  fprintf(stderr, "base: %s, transform: %s\n", base, transform);

  // 查看是否起用电子热力学
  int electrons_on = get_param_int(par_string, "electrons", "on", 0);
  if (electrons_on) {
    fprintf(stderr, "Electron thermodynamics is enabled.\n");
  } else {
    fprintf(stderr, "Electron thermodynamics is disabled.\n");
    ELECTRONS = 0;
  }

  // 暂时跳过了关于radiation, weights, derefine_poles的代码
  RADIATION = 0;

  // 根据base和transform确定metric
  metric = 0;
  use_eKS_internal = 0;
  ...
  if (strcmp(transform, "mks") == 0) {
    metric = METRIC_MKS;
    cstopx[2] = 1.0;
    hslope = get_param_double(par_string, "coordinates", "hslope", 0.3);
    fprintf(stderr, "MKS parameters a: %f hslope: %f Rin: %f Rout: %f\n", a, hslope, Rin, Rout);
  } else if (strcmp(transform, "fmks") == 0) {
    metric = METRIC_FMKS;
    cstopx[2] = 1.0;
    hslope = get_param_double(par_string, "coordinates", "hslope", 0.3);
    poly_xt = get_param_double(par_string, "coordinates", "poly_xt", 0.82);
    poly_alpha = get_param_double(par_string, "coordinates", "poly_alpha", 14.0);
    mks_smooth = get_param_double(par_string, "coordinates", "mks_smooth", 0.5);
    poly_norm = 0.5*M_PI*1./(1. + 1./(poly_alpha + 1.)*1./pow(poly_xt, poly_alpha));
    fprintf(stderr, "FMKS parameters a: %f hslope: %f poly_xt: %f poly_alpha: %f mks_smooth: %f poly_norm: %f\n", a, hslope, poly_xt, poly_alpha, mks_smooth, poly_norm);
  } else{
    fprintf(stderr, "Unsupported KHARMA coordinate transform %s. Cannot continue.\n", transform);
    exit(-1);
  }
  ...
  // 读取转储步长
  DTd = get_param_double(par_string, "parthenon/output0", "dt", 5.0);
  fprintf(stderr, "Dump cadence set to %f\n", DTd);

  // 从参数字符串中读取网格信息
  N1 = get_param_int(par_string, "parthenon/mesh", "nx1", 128);
  N2 = get_param_int(par_string, "parthenon/mesh", "nx2", 128);
  N3 = get_param_int(par_string, "parthenon/mesh", "nx3", 128);
  
  // 读取其他参数
  gam = get_param_double(par_string, "GRMHD", "gamma", 1.666667);

  //跳过关于custom electron model的代码
  ...
  // 跳过关于radiation units的代码

  // 设置网格参数
  startx[1] = get_param_double(par_string, "parthenon/mesh", "x1min", log(Rin));
  stopx[1] = get_param_double(par_string, "parthenon/mesh", "x1max", log(Rout));
  startx[2] = get_param_double(par_string, "parthenon/mesh", "x2min", 0.0);
  startx[3] = get_param_double(par_string, "parthenon/mesh", "x3min", 0.0);

  if (metric == METRIC_MKS || metric == METRIC_FMKS) {
    stopx[2] = get_param_double(par_string, "parthenon/mesh", "x2max", 1.0);
  } else {
    stopx[2] = get_param_double(par_string, "parthenon/mesh", "x2max", M_PI);
  }
  
  stopx[3] = get_param_double(par_string, "parthenon/mesh", "x3max", 2*M_PI);
  ...
  // char *single_precision_output_str = get_param_string(par_string, "parthenon/output0", "single_precision_output", "false");
  // if (strcmp(single_precision_output_str, "true") == 0) {
  //   single_precision_output = true;
  //   fprintf(stderr, "Single precision output is enabled.\n");
  // } else {
  //   single_precision_output = false;
  //   fprintf(stderr, "Single precision output is disabled.\n");
  // }
...
void load_data(int n, char *fnam, int dumpidx, int verbose)
{
  if (dumpfile_format == FORMAT_IHARM_v1) {
    ...
    load_kharma_data(n, fnam, dumpidx, verbose);
  }
...
#pragma omp parallel for collapse(2) reduction(+:dMact,Ladv)
  for(int i = 1; i < N1+1; i++) {
    for(int j = 1; j < N2+1; j++) {
      ...
      //fprintf(stderr, "HAMR %d %d %g %g %g %g %g %g %g %g %g %g %g %g %g\n", i,j,r,th,
...
void load_iharm_data(int n, char *fnam, int dumpidx, int verbose)
{
  ...
  // 定义dMact和Ladv，其中dMact是质量变化，Ladv是辐射能量变化
  double dMact, Ladv;
...
{
  fprintf(stderr, "Loading KHARMA data from %s\n", fnam);
  double dMact, Ladv;
  char fname[256];
  snprintf(fname, 255, fnam, dumpidx);
  nloaded++;
  ...
  // 读取meshblock信息
  hsize_t dims[2];
  int *logical_locations;
  int n_meshblocks;

  // 读取每个meshblock的大小
  int meshblock_size[3]; // [n1, n2, n3] for i,j,k

  // 设置目录到Info组
  hdf5_set_directory("/Info");

  // 读取MeshBlockSize属性数组
  if (hdf5_read_attr_arr(meshblock_size, "MeshBlockSize", "") < 0) {
    fprintf(stderr, "Error reading MeshBlockSize attribute array\n");
    exit(-1);
  }

  // 打印读取的值用于调试
  fprintf(stderr, "MeshBlockSize: [%d, %d, %d]\n", 
          meshblock_size[0], meshblock_size[1], meshblock_size[2]);

  int n1_block = meshblock_size[0]; // i方向
  int n2_block = meshblock_size[1]; // j方向
  int n3_block = meshblock_size[2]; // k方向

  // 计算每个方向上meshblock的数量
  int nblocks_i = N1/n1_block;
  int nblocks_j = N2/n2_block;
  int nblocks_k = N3/n3_block;

  hdf5_set_directory("/");

  // 读取LogicalLocations数组
  n_meshblocks = nblocks_i * nblocks_j * nblocks_k;
  fprintf(stderr, "n_meshblocks: %d\n", n_meshblocks);
  logical_locations = (int*)malloc(n_meshblocks * 3 * sizeof(int));

  // 设置正确的维度和起始位置
  hsize_t fdims_logical_locations[] = {n_meshblocks, 3};  // 文件中数据的维度
  hsize_t fstart_logical_locations[] = {0, 0};            // 从哪里开始读
  hsize_t fcount_logical_locations[] = {n_meshblocks, 3}; // 读取多少
  hsize_t mdims_logical_locations[] = {n_meshblocks, 3};  // 内存中数据的维度
  hsize_t mstart_logical_locations[] = {0, 0};            // 内存中的起始位置

  // 读取数据
  if (hdf5_read_array(logical_locations, "LogicalLocations", 2, 
                      fdims_logical_locations, fstart_logical_locations, fcount_logical_locations, 
                      mdims_logical_locations, mstart_logical_locations, H5T_STD_I32LE) < 0) {
    fprintf(stderr, "Error reading LogicalLocations dataset\n");
    free(logical_locations);
    exit(-1);
  }

  // for(int i = 0; i < n_meshblocks; i++) {
  //   fprintf(stderr, "LogicalLocations[%d]: [%d, %d, %d]\n", 
  //           i, logical_locations[i*3], logical_locations[i*3+1], logical_locations[i*3+2]);
  // }
  ...
  // 分配临时缓冲区用于读取单个meshblock的数据
  double *block_data = malloc(n1_block * n2_block * n3_block * sizeof(double));

  // 读取密度 rho: shape (n_blocks, n3, n2, n1)
  hsize_t fdims[] = {n_meshblocks, n3_block, n2_block, n1_block};
  ...
  for(int mb = 0; mb < n_meshblocks; mb++) {
    ...
    // 计算该block在全局网格中的起始位置
    int i_start = logical_locations[mb*3 + 0] * n1_block + 1; // +1 for ghost zones
    int j_start = logical_locations[mb*3 + 1] * n2_block + 1;
    int k_start = logical_locations[mb*3 + 2] * n3_block + 1;

    // 读取密度
    hdf5_read_array(block_data, "/prims.rho", 4, fdims, fstart, count, mdims, mstart, H5T_IEEE_F64LE);

    // 输出block_data[:,0,0]的数据
    // for(int k = 0; k < n3_block; k++) {
    //   fprintf(stderr, "%g \t", block_data[0 + n1_block * (0 + n2_block * k)]);
    // }
    // fprintf(stderr, "\n");
    ...
    //     for(int k = 0; k < n3_block; k++){
    //       data[n]->p[KRHO][i_start+i][j_start+j][k_start+k] = 
    //         block_data[k + n3_block*(j + n2_block*i)];
    //       // fprintf(stderr, "data[n]->p[KRHO][%d][%d][%d] = %g\n", i_start+i, j_start+j, k_start+k, data[n]->p[KRHO][i_start+i][j_start+j][k_start+k]);
    //     }
    for(int i = 0; i < n1_block; i++)
      for(int j = 0; j < n2_block; j++)
        for(int k = 0; k < n3_block; k++) {
          int index = i + n1_block * (j + n2_block * k);
          data[n]->p[KRHO][i_start + i][j_start + j][k_start + k] = block_data[index];
        }
          
    // 读取内能
    hdf5_read_array(block_data, "/prims.u", 4, fdims, fstart, count, mdims, mstart, H5T_IEEE_F64LE);
    for(int i = 0; i < n1_block; i++)
      for(int j = 0; j < n2_block; j++)
        for(int k = 0; k < n3_block; k++) {
          int index = i + n1_block * (j + n2_block * k);
          data[n]->p[UU][i_start + i][j_start + j][k_start + k] = block_data[index];
        }

    // 读取速度分量 uvec: shape (n_blocks, 3, n3, n2, n1)
    hsize_t uvec_dims[] = {n_meshblocks, 3, n3_block, n2_block, n1_block};
    ...
    for(int v = 0; v < 3; v++) {
      ...
      // if (v == 1) {
      //     for (int k = 0; k < n3_block; k++) {
      //         int index = 0 + n1_block * (0 + n2_block * k);
      //         fprintf(stderr, "%g \t", block_data[index]);
      //     }
      //     fprintf(stderr, "\n");
      // }
      ...
      for(int i = 0; i < n1_block; i++)
        for(int j = 0; j < n2_block; j++)
          for(int k = 0; k < n3_block; k++) {
            int index = i + n1_block * (j + n2_block * k);
            data[n]->p[U1+v][i_start + i][j_start + j][k_start + k] = block_data[index];
          }
    }

    // 读取磁场分量 B: shape (n_blocks, 3, n3, n2, n1)
    hsize_t b_dims[] = {n_meshblocks, 3, n3_block, n2_block, n1_block};
    ...
    for(int v = 0; v < 3; v++) {
      ...
      for(int i = 0; i < n1_block; i++)
        for(int j = 0; j < n2_block; j++)
          for(int k = 0; k < n3_block; k++) {
            int index = i + n1_block * (j + n2_block * k);
            data[n]->p[B1+v][i_start + i][j_start + j][k_start + k] = block_data[index];
          }
    }
  }
  ...
  if(ELECTRONS == ELECTRONS_TFLUID || ELECTRONS == 1) {
    fprintf(stderr, "ELECTRONS = %d not supported yet\n", ELECTRONS);
    exit(-1);
  }
  ...
  // 读取Time属性
  if (hdf5_read_attr_num(&(data[n]->t), "Time", "", H5T_IEEE_F64LE) < 0) {
    fprintf(stderr, "Error reading Time attribute from Info group\n");
    exit(-1);
  }
  ...
  // for(int i = 0; i < N1+2; i++) {
  //   for(int j = 0; j < N2+2; j++) {
  //     fprintf(stderr, "%g \t", data[n]->p[KRHO][i][j][0]);
  //   }
  //   fprintf(stderr, "\n");
  // }
